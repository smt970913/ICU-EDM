...
class DataLoader:
    def __init__(self, cfg, state_id_path, rl_state_path, test_size = 0.20, random_state = 68, scaler = StandardScaler()):
        self.cfg = cfg
    ...
    def data_buffer_test(self):
        self.test_memory = OCRL_model_fqi.ReplayBuffer(self.cfg.memory_capacity)

        for i in range(len(self.test_state_table)):

            state = self.rl_state_var_sc_test.values[i]
            action = self.test_state_table['EXT'].values[i]
            obj_cost = self.test_state_table['obj_costs'].values[i]
            con_cost = self.test_state_table['con_costs'].values[i]
            done = self.test_state_table['EXT'].values[i]
    ...
    def data_torch_loader_test(self):
        ...
        constraint_num = len(con_cost_batch)

        con_cost_batch_dict = {i: [] for i in range(constraint_num)}
        for m in range(constraint_num):
            con_cost_batch_dict[m] = tuple(con_cost_batch[m])
